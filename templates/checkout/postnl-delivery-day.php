				...
				<ul class="postnl_sub_list">
					<?php foreach ( $delivery['options'] as $option_index => $option ) { ?>
						<?php
							...
							$is_charged    = ( empty( $option['price'] ) || $option['price'] <= 0 ) ? '' : '+' . \PostNLWooCommerce\Utils::get_formatted_fee_total_price( $option['price'] );
						...
						<li
							...
							data-price-formatted="<?php echo esc_attr( \PostNLWooCommerce\Utils::get_formatted_fee_total_price( $option['price'] ) ); ?>"
